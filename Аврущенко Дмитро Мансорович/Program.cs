...
namespace ASD1_AvruschenkoTest
{
    internal class Program
    {
        ...
        static void Linear(Node list, int element)
        {
            ...
            Node listch = list.head;
            int i = 0;
            while (listch != null)
            {
                
                if (listch.Data == element)
                    break;
                listch = listch.next_ind;
                i++;
            }
        ...
        static void Barrier(int[] arr, int element)
        {
            ...
            Console.WriteLine($"You used for array with {arrch.Length - 2} elements {timer.Elapsed}");
        }
        static void BarrierList(int[] arr, int element)
        {
            var list = new Node();
            for (int j = 0; j < arr.Length; j++) { list.AddLast(arr[j]); }
            var sw = Stopwatch.StartNew();
            list.AddLast(element);
           
            Node listch = list.head;
            int i = 0;
            while (listch.Data != element)
            {
                listch = listch.next_ind;
                i++;
            }
            ...
            sw.Stop();
            if (i == list.Count-1) { Console.WriteLine("The element is not found"); }
            else { Console.WriteLine("The element has index: " + i); }
            Console.WriteLine($"You used for list with {list.Count - 2} elements {sw.Elapsed}");
        }
